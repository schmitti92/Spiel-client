(() => {
  ...
  const S = {
    ...
    // Render/Animation (UX only)
    pieceRender: new Map(), // pieceId -> {x,y} in board coords
    pieceAnim: new Map(),   // pieceId -> {fx,fy,tx,ty,t0,dur,hop}
    rafId: 0,
...
async function loadBoard(){
  ...
  function resetPieces(){
    ...
    // init render positions (board coords)
    S.pieceRender = new Map();
    S.pieceAnim = new Map();
    for (const pc of S.pieces){
      const n = S.nodeById.get(pc.nodeId);
      if (n) S.pieceRender.set(pc.id, {x:n.x, y:n.y});
    }
  }

  // =========================
  // High-end piece animation (UX only)
  // =========================
  ...
  function startAnimLoop(){
    ...
    const tick = ()=>{
      S.rafId = 0;
      draw();
      if (S.pieceAnim.size) startAnimLoop();
    };
    S.rafId = requestAnimationFrame(tick);
  }

  function animatePieceTo(pieceId, toNodeId, opts={}){
    const nTo = S.nodeById.get(toNodeId);
    if (!nTo) return;
    const cur = S.pieceRender.get(pieceId);
    const fx = cur?.x ?? nTo.x;
    const fy = cur?.y ?? nTo.y;
    const tx = nTo.x;
    const ty = nTo.y;
    const now = performance.now();
    const dur = Math.max(160, Math.min(520, opts.dur ?? 260));
    const hop = opts.hop ?? 12;
    S.pieceAnim.set(pieceId, {fx,fy,tx,ty,t0:now,dur,hop});
    S.pieceRender.set(pieceId, {x:fx,y:fy});
    startAnimLoop();
  }
  ...
  function tryStepTo(targetNodeId){
    ...
    const fromId = piece.nodeId;
    piece.nodeId = targetNodeId;
    // Animate the move (UX only)
    animatePieceTo(piece.id, targetNodeId, {dur: 240, hop: 14});
    S.stepsLeft -= 1;
  ...
  function draw(){
    ...
    // pieces (high-end look + animation)
    const now = performance.now();
    for (const pc of S.pieces){
      const n = S.nodeById.get(pc.nodeId);
      if (!n) continue;
      ...
      // Resolve animated position in BOARD coords
      let bx = n.x, by = n.y, hopY = 0;
      const anim = S.pieceAnim.get(pc.id);
      if (anim){
        const t = clamp((now - anim.t0) / anim.dur, 0, 1);
        const e = easeInOutCubic(t);
        bx = anim.fx + (anim.tx - anim.fx) * e;
        by = anim.fy + (anim.ty - anim.fy) * e;
        hopY = Math.sin(Math.PI * e) * (anim.hop || 0);
        S.pieceRender.set(pc.id, {x:bx, y:by});
        if (t >= 1){
          S.pieceAnim.delete(pc.id);
          S.pieceRender.set(pc.id, {x:anim.tx, y:anim.ty});
        }
      } else {
        const r = S.pieceRender.get(pc.id);
        if (!r){
          S.pieceRender.set(pc.id, {x:n.x, y:n.y});
        } else {
          bx = r.x; by = r.y;
        }
      }

      drawHighEndPiece(ctx, X(bx), Y(by) - hopY, pc.color, sel, CAM.scale);
    }
  }

  function drawHighEndPiece(ctx, sx, sy, color, selected, scale){
    // scale-aware sizing (keeps the piece readable at any zoom)
    const r = clamp(10 / Math.sqrt(Math.max(0.35, scale)), 7.5, 12);
    const glow = r * 1.9;

    // soft drop shadow
    ctx.save();
    ctx.beginPath();
    ctx.ellipse(sx, sy + r*0.9, r*1.15, r*0.55, 0, 0, Math.PI*2);
    ...
    ctx.fillStyle = colorTo(color, 0.14);
    ctx.fill();
    ctx.restore();

    // body (radial gradient)
    const g = ctx.createRadialGradient(sx - r*0.35, sy - r*0.35, r*0.2, sx, sy, r*1.25);
    g.addColorStop(0, colorTo(color, 0.98));
    g.addColorStop(0.6, colorTo(color, 0.86));
    g.addColorStop(1, "rgba(2,6,23,.95)");
    ctx.beginPath();
    ...
    // glossy highlight
    ctx.beginPath();
    ctx.ellipse(sx - r*0.18, sy - r*0.35, r*0.55, r*0.38, -0.35, 0, Math.PI*2);
    ...
    if (selected){
      ...
      ctx.beginPath();
      ctx.arc(sx, sy, r*1.55, 0, Math.PI*2);
      ctx.strokeStyle = colorTo(color, 0.55);
      ctx.lineWidth = 5.5;
      ctx.stroke();
    }
  }
