...
function hideWheel(){
  ...
  // ===== Hybrid 3D Dice Renderer (Canvas 2D pseudo‑3D, WebGL‑free, Samsung‑Internet safe) =====
  // Visual only: does NOT change any game logic. We keep the existing CSS dice as fallback.
  const dice3D = (() => {
    let enabled = false;
    ...
    // animation state
    let raf = 0;
    let anim = null; // {t0, dur, a0,b0,g0, a1,b1,g1, face}
    let angles = { a: 0.6, b: 0.9, g: 0.15 }; // rx, ry, rz

    // mapping: face -> target Euler angles (rx, ry, rz)
    // These are hand‑tuned to look natural; we also add a tiny random wobble at the end.
    const targets = {
      1: { a: 0,          b: 0,          g: 0 },
      2: { a: -Math.PI/2, b: 0,          g: 0 },
      3: { a: 0,          b:  Math.PI/2, g: 0 },
      4: { a: 0,          b: -Math.PI/2, g: 0 },
      5: { a:  Math.PI/2, b: 0,          g: 0 },
      6: { a:  Math.PI,   b: 0,          g: 0 },
    };
    ...
    function ensureCanvas(){
      if(!diceEl) return false;
      if(canvas && ctx) return true;
      try{
        canvas = document.createElement("canvas");
        canvas.id = "dice3dCanvas";
        ...
        canvas.style.borderRadius = "16px"; // matches your dice rounding
        canvas.style.overflow = "hidden";
        ctx = canvas.getContext("2d", { alpha: true });
        if(!ctx) return false;

        // Make sure the parent is positioning context
        try{
          const cs = getComputedStyle(diceEl);
          if(cs.position === "static") diceEl.style.position = "relative";
        }catch(_e){}
        // Insert canvas on top
        diceEl.appendChild(canvas);

        // Hide existing CSS dice faces, but keep element sizing/layout intact.
        try{
          for(const ch of Array.from(diceEl.children)){
            if(ch !== canvas){ ch.style.opacity = "0"; ch.style.pointerEvents = "none"; }
          }
        }catch(_e){}
    ...
    function resize(){
      if(!canvas || !diceEl) return;
      const rect = diceEl.getBoundingClientRect();
      const ww = Math.max(40, rect.width || 0);
      const hh = Math.max(40, rect.height || 0);
    ...
    function rotXYZ(p, a, b, g){
      // rotate around X (a), Y (b), Z (g)
      let {x,y,z} = p;
      // X
      let cy = Math.cos(a), sy = Math.sin(a);
      let y1 = y*cy - z*sy;
      let z1 = y*sy + z*cy;
      y = y1; z = z1;
      // Y
      ...
      x = x2; z = z2;
      // Z
      let cz = Math.cos(g), sz = Math.sin(g);
      let x3 = x*cz - y*sz;
      let y3 = x*sz + y*cz;
      x = x3; y = y3;
      return {x,y,z};
    ...
    function facePips(face){
      // 3x3 positions in a centered inner square (authentic spacing)
      // coords in [-1,1] plane
      const s = 0.58; // inner grid half-size (smaller -> more centered pips)
      const L = -s, C = 0, R = s;
      const T = -s, M = 0, B = s;
      const map = {
        1: [[C,M]],
    ...
    function drawPip(px, py, r){
      // "inset" pip: outer soft edge + inner shadow (like a drilled hole)
      ctx.beginPath();
      ...
      const grd = ctx.createRadialGradient(px-r*0.2, py-r*0.2, r*0.2, px, py, r);
    ...
    function drawFace(poly, shade, face){
      // poly: array of {x,y} in screen space (px)
      // shade: 0..1, face: 1..6
      // base material (light glossy plastic)
      const minShade = 0.25;
      const s = clamp(shade, 0, 1);
      const body = `rgba(245,246,248,${1})`;
      const edge = `rgba(0,0,0,${0.12 + (1-s)*0.08})`;

      // face fill with subtle lighting
      ctx.save();
      ctx.beginPath();
      ...
      // lighting gradient
      const bb = poly.reduce((acc,p)=>({
        minx: Math.min(acc.minx,p.x), miny: Math.min(acc.miny,p.y),
        maxx: Math.max(acc.maxx,p.x), maxy: Math.max(acc.maxy,p.y)
      }), {minx:1e9,miny:1e9,maxx:-1e9,maxy:-1e9});
      const gx0 = bb.minx, gy0 = bb.miny, gx1 = bb.maxx, gy1 = bb.maxy;

      const g = ctx.createLinearGradient(gx0, gy0, gx1, gy1);
      g.addColorStop(0, `rgba(255,255,255,${0.95})`);
      g.addColorStop(0.55, `rgba(245,246,248,${0.92})`);
      g.addColorStop(1, `rgba(214,218,225,${0.95})`);
      ctx.fillStyle = g;
      ctx.fill();

      // shading overlay (directional)
      ctx.globalCompositeOperation = "multiply";
      ctx.fillStyle = `rgba(0,0,0,${(1-s)*0.35})`;
      ctx.fill();
      ...
      // glossy highlight (only on the most lit faces)
      if(s > 0.62){
        ctx.save();
        ctx.globalCompositeOperation = "screen";
        const hg = ctx.createRadialGradient(
          (gx0*0.7+gx1*0.3), (gy0*0.7+gy1*0.3), 2,
          (gx0*0.7+gx1*0.3), (gy0*0.7+gy1*0.3), Math.max(16, (gx1-gx0)*0.9)
        );
      ...
      // pips (projected inside face via barycentric-ish local mapping)
      // We'll approximate by mapping pips onto the face plane using two axes from the polygon.
      // pick top-left, top-right, bottom-left to form local basis
      const p0 = poly[0], p1 = poly[1], p3 = poly[3] || poly[2]; // robust
      const ux = { x: (p1.x - p0.x), y: (p1.y - p0.y) };
      const vx = { x: (p3.x - p0.x), y: (p3.y - p0.y) };

      // pip radius based on face size
      const faceW = Math.hypot(ux.x, ux.y);
      const r = Math.max(2.2, faceW * 0.06);

      // pip color (near-black with slight softness)
      ctx.fillStyle = "rgba(15,16,18,1)";

      const pips = facePips(face);
      for(const [gx, gy] of pips){
        // gx,gy in [-1..1] inner grid, convert to [0..1] in local face coords with padding
        const u = 0.5 + gx*0.22;
        const v = 0.5 + gy*0.22;
        const px = p0.x + ux.x*u + vx.x*v;
        const py = p0.y + ux.y*u + vx.y*v;
        drawPip(px, py, r);
      }
    ...
    function draw(){
      if(!enabled || !ctx || !diceEl) return;

      resize();

      const rect = diceEl.getBoundingClientRect();
      const ww = rect.width || 160;
      const hh = rect.height || 160;
      ...
      ctx.save();
      ctx.globalCompositeOperation = "source-over";
      const shadow = ctx.createRadialGradient(ww*0.52, hh*0.86, 6, ww*0.52, hh*0.86, ww*0.42);
      shadow.addColorStop(0, "rgba(0,0,0,0.28)");
      shadow.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = shadow;
      ctx.beginPath();
      ctx.ellipse(ww*0.52, hh*0.86, ww*0.34, hh*0.14, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.restore();
      ...
      // faces as indices + dice face number mapping
      const faces = [
        { idx:[4,5,6,7], face: 1, n:{x:0,y:0,z:1} },   // front
        { idx:[0,1,2,3], face: 6, n:{x:0,y:0,z:-1} },  // back
        { idx:[1,5,6,2], face: 3, n:{x:1,y:0,z:0} },   // right
        { idx:[0,4,7,3], face: 4, n:{x:-1,y:0,z:0} },  // left
        { idx:[3,2,6,7], face: 2, n:{x:0,y:1,z:0} },   // top
        { idx:[0,1,5,4], face: 5, n:{x:0,y:-1,z:0} },  // bottom
      ];

      // rotate and project vertices
      const rverts = verts.map(v => rotXYZ(v, angles.a, angles.b, angles.g));
      const pverts = rverts.map(v => project(v));

      // light direction
      const L = (() => {
        const lx= -0.4, ly=-0.8, lz= 1.2;
        const m = Math.hypot(lx,ly,lz) || 1;
        return {x:lx/m, y:ly/m, z:lz/m};
      })();

      // compute face depth + visibility + shade
      const faceDraw = [];
      for(const f of faces){
        const n = rotXYZ(f.n, angles.a, angles.b, angles.g);
        // face facing camera if z component > 0 (approx)
        const vis = n.z > 0.06;
        if(!vis) continue;

        // shade by dot(light, normal)
        const dot = (n.x*L.x + n.y*L.y + n.z*L.z);
        const shade = clamp((dot+1)/2, 0, 1);

        // avg depth
        const zavg = f.idx.reduce((acc,i)=>acc + rverts[i].z, 0) / f.idx.length;

        // polygon in screen space
        const poly = f.idx.map(i=>{
          const pv = pverts[i];
          return {
            x: ww*0.5 + pv.x * ww*0.38,
            y: hh*0.5 + pv.y * hh*0.38,
          };
        });

        faceDraw.push({ poly, shade, zavg, face: f.face });
      }

      // draw back-to-front
      faceDraw.sort((a,b)=> a.zavg - b.zavg);
      for(const f of faceDraw){
        drawFace(f.poly, f.shade, f.face);
      }
    }
    ...
    function startLoop(){
      ...
      const loop = () => {
        ...
        if(anim){
          ...
          angles.a = lerp(anim.a0, anim.a1, e);
          angles.b = lerp(anim.b0, anim.b1, e);
          angles.g = lerp(anim.g0, anim.g1, e);
          if(t >= 1){
            angles.a = anim.a1; angles.b = anim.b1; angles.g = anim.g1;
            anim = null;
          }
        }
        draw();
      ...
      window.addEventListener("resize", () => { try{ resize(); }catch(_e){} }, { passive:true });
    }

    function rollTo(face){
      if(!ensureCanvas()) return false;
      const tgt = targets[face] || targets[1];

      // Add a big spin before settling (visual only)
      const spinA = (Math.random()*2+2) * Math.PI;
      const spinB = (Math.random()*2+2) * Math.PI;
      const spinG = (Math.random()*2+2) * Math.PI;

      const wob = () => (Math.random()-0.5)*0.10;

      anim = {
        t0: now(),
        dur: 950,  // feels snappy; your 10s wheel is separate
        a0: angles.a, b0: angles.b, g0: angles.g,
        a1: tgt.a + spinA + wob(),
        b1: tgt.b + spinB + wob(),
        g1: tgt.g + spinG + wob(),
        face
    ...
    function setFaceInstant(face){
      if(!ensureCanvas()) return false;
      const tgt = targets[face] || targets[1];
      angles.a = tgt.a; angles.b = tgt.b; angles.g = tgt.g;
      anim = null;
      draw();
      return true;
    }
...
function toast(msg){
  ...
  function setDiceFaceAnimated(v){
    ...
    // Prefer Hybrid 3D dice renderer when available (visual only)
    // Fallback remains the existing CSS dice.
    try{ dice3D.ensure(); }catch(_e){}
    ...
    // Hybrid 3D visual: animate the cube to the final face (no gameplay impact)
    if(face!==0){
      try{
        if(dice3D && dice3D.enabled){
          dice3D.rollTo(face);
        }
      }catch(_e){}
    }
