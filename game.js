...
try{ ws = new WebSocket(SERVER_URL); }
  ...
  function applyRemoteState(remote){
    ...
    if(st.turnColor && Array.isArray(st.pieces) && Array.isArray(st.barricades)){
      const server = st;
      const players = Array.isArray(server.turnOrder) && server.turnOrder.length
        ? server.turnOrder.slice()
        : ["red","blue"];
      setPlayers(players);
      const ALL_COLORS = ["red","yellow","green","blue"];
      const piecesByColor = {red:[], yellow:[], green:[], blue:[]};
      // Always keep 4 colors visible (also offline / before selection)
